   ...
   // --- Attach Event Listeners ---
    ...
    if (searchInputSocialLinks) {
        searchInputSocialLinks.addEventListener('input', displayFilteredSocialLinks);
          ...
          // --- Variable Declarations for Disabilities Feature ---
    // Get references to all necessary elements once the DOM is loaded.
    const disabilitiesListAdmin = document.getElementById('disabilities-list-admin');
    ...
    const editDisabilityUrlInput = document.getElementById('edit-disability-url');
    const editDisabilityOrderInput = document.getElementById('edit-disability-order');
    const editDisabilityIconInput = document.getElementById('edit-disability-icon'); // For edit modal icon field
    const editDisabilityStatusMessage = document.getElementById('edit-disability-status-message');

    // Global array reference (assuming allDisabilities is declared in a higher scope or replace with a local declaration if not)
    // let allDisabilities = []; // Uncomment this line if allDisabilities is not declared globally in your script.

    // --- Core Functions ---

    /**
     * Renders a single Disability Link item in the admin list.
     * @param {HTMLElement} container - The list container element.
     * @param {string} docId - Firestore document ID.
     * @param {string} name - Disability name/label.
     * @param {string} url - Information URL.
     * @param {number} order - Display order number.
     * @param {string} iconClass - Font Awesome icon class name.
     * @param {function} deleteHandler - Function to call on delete click.
     * @param {function} editHandler - Function to call on edit click.
     */
    function renderDisabilityAdminListItem(container, docId, name, url, order, iconClass, deleteHandler, editHandler) {
        ...
        // Use a default icon if iconClass is missing or empty
        const effectiveIconClass = iconClass || 'fa-universal-access';

        itemDiv.innerHTML = `
            <div class="item-content">
                <div class="item-details">
                    <span class="disability-icon" style="margin-right: 8px; width: 20px; text-align: center;">
    ...
    /**
     * Shows status messages inside the Edit Disability modal.
    ...
    /**
     * Loads disability links from Firestore, populates the global array, and triggers display rendering.
     */
    async function loadDisabilitiesAdmin() {
        ...
        try {
            // Assumes disabilitiesCollectionRef is defined and imported correctly from firebase-init.js scope
            ...
            if (error.code === 'failed-precondition') {
                ...
                // Assumes showAdminStatus function exists elsewhere in your script
                if(typeof showAdminStatus === 'function') showAdminStatus(errorMsg, true);
            } else {
                if(typeof showAdminStatus === 'function') showAdminStatus(errorMsg + `: ${error.message}`, true);
            }
    ...
    /**
     * Handles adding a new disability link from the form submission.
    ...
    async function handleAddDisability(event) {
        ...
        const iconInput = addDisabilityForm.querySelector('#disability-icon'); // For add form icon field
        ...
        if (!name || !url || !orderStr || isNaN(order) || order < 0) {
            if(typeof showAdminStatus === 'function') showAdminStatus("Invalid input. Name, URL, and non-negative Order required.", true);
            return;
        }
        try { new URL(url); } catch (_) {
            if(typeof showAdminStatus === 'function') showAdminStatus("Invalid URL format.", true);
            return;
        }

        const disabilityData = { name, url, order, iconClass, createdAt: serverTimestamp() };

        if(typeof showAdminStatus === 'function') showAdminStatus("Adding disability link...");
        try {
            await addDoc(disabilitiesCollectionRef, disabilityData);
            if(typeof showAdminStatus === 'function') showAdminStatus("Disability link added successfully.", false);
            addDisabilityForm.reset();
            loadDisabilitiesAdmin();
        } catch (error) {
            console.error("Error adding disability link:", error);
            if(typeof showAdminStatus === 'function') showAdminStatus(`Error adding link: ${error.message}`, true);
        }
    }

    /**
     * Handles deleting a specified disability link.
    ...
    async function handleDeleteDisability(docId) {
        ...
        if(typeof showAdminStatus === 'function') showAdminStatus("Deleting disability link...");
        try {
            await deleteDoc(doc(db, 'disabilities', docId));
            if(typeof showAdminStatus === 'function') showAdminStatus("Disability link deleted successfully.", false);
            loadDisabilitiesAdmin();
        } catch (error) {
            console.error(`Error deleting disability link (ID: ${docId}):`, error);
            if(typeof showAdminStatus === 'function') showAdminStatus(`Error deleting link: ${error.message}`, true);
        }
    }

    /**
     * Opens and populates the Edit Disability modal with data for the selected item.
     * @param {string} docId - The Firestore document ID to edit.
     */
    function openEditDisabilityModal(docId) {
        if (!editDisabilityModal || !editDisabilityForm) {
            ...
            if(typeof showAdminStatus === 'function') showAdminStatus("UI Error: Cannot open edit form.", true);
            return;
        }
        ...
        getDoc(docRef).then(docSnap => {
            if (docSnap.exists()) {
                ...
                if (editDisabilityIconInput) editDisabilityIconInput.value = data.iconClass || 'fa-universal-access';

                // Update icon preview if elements exist
                const previewDiv = document.getElementById('edit-disability-icon-preview');
                if (previewDiv) updateDisabilityIconPreview(editDisabilityIconInput, previewDiv);

                editDisabilityModal.style.display = 'block';
                showEditDisabilityStatus("");
            } else {
                if(typeof showAdminStatus === 'function') showAdminStatus("Error: Could not load data for editing.", true);
                showEditDisabilityStatus("Error: Link not found.", true);
            }
        }).catch(error => {
            console.error("Error getting disability document for edit:", error);
            if(typeof showAdminStatus === 'function') showAdminStatus(`Error loading data: ${error.message}`, true);
            showEditDisabilityStatus(`Error: ${error.message}`, true);
        });
    }

    /**
     * Closes the Edit Disability modal and resets form state.
     */
    ...
    /**
     * Handles saving updates from the Edit Disability modal form.
     * @param {Event} event - The form submit event.
     */
    async function handleUpdateDisability(event) {
        ...
        const order = parseInt(orderStr);
        const iconClass = editDisabilityIconInput?.value.trim() || 'fa-universal-access';

        if (!name || !url || !orderStr || isNaN(order) || order < 0) {
            showEditDisabilityStatus("Invalid input. Name, URL, and non-negative Order required.", true);
            return;
        }
        try { new URL(url); } catch (_) {
        ...
        const newDataFromForm = { name, url, order, iconClass, lastModified: serverTimestamp() };
        showEditDisabilityStatus("Saving changes...");
        const docRef = doc(db, 'disabilities', docId);

        try {
            await updateDoc(docRef, newDataFromForm);
            if(typeof showAdminStatus === 'function') showAdminStatus("Disability link updated successfully.", false);
            closeEditDisabilityModal();
            loadDisabilitiesAdmin(); // Reload data to reflect changes
        } catch (error) {
     ...
     * @param {HTMLInputElement} inputElement - The text input containing the icon class.
     * @param {HTMLElement} previewElement - The container for the preview icon.
     */
    function updateDisabilityIconPreview(inputElement, previewElement) {
        if (!inputElement || !previewElement) return;
        const iconClass = inputElement.value.trim();
        const iconElement = previewElement.querySelector('i');
        if (iconElement) {
            iconElement.className = `fas ${iconClass || 'fa-universal-access'}`;
        }
    }

    /**
     * Sets up event listeners for disability icon input/select elements to update preview.
     * @param {string} formIdPrefix - 'add' or 'edit'.
     */
    function setupDisabilityIconListeners(formIdPrefix) {
        const iconInput = document.getElementById(`${formIdPrefix}-disability-icon`);
        const iconSelect = document.getElementById(`${formIdPrefix}-disability-icon-select`);
        const iconPreview = document.getElementById(`${formIdPrefix}-disability-icon-preview`);

        if (iconInput && iconPreview) {
            iconInput.addEventListener('input', () => {
                updateDisabilityIconPreview(iconInput, iconPreview);
            });
        }
        if (iconSelect && iconInput && iconPreview) {
    ...
    /**
     * Filters and displays disability items based on search criteria.
     */
    function displayFilteredDisabilities() {
        ...
        const listToRender = allDisabilities.filter(item => {
            if (!searchTerm) return true;
        ...
        if (listToRender.length > 0) {
            listToRender.forEach(item => {
                renderDisabilityAdminListItem(
                    disabilitiesListAdmin, item.id, item.name, item.url, item.order, item.iconClass,
    ...
    if (addDisabilityForm) {
        ...
        setupDisabilityIconListeners('add'); // Setup for add form preview
    }

    if (editDisabilityForm) {
        editDisabilityForm.addEventListener('submit', handleUpdateDisability);
        setupDisabilityIconListeners('edit'); // Setup for edit form preview
    }

    // Load initial data for disabilities (assuming loadDisabilitiesAdmin is called on login success)
    // loadDisabilitiesAdmin(); // Call this when appropriate (e.g., after login)
